...
// ESKF 节点：IMU 预测 + 位姿观测更新（姿态观测与位置观测）。
class EskfNode : public rclcpp::Node {
 public:
  EskfNode() : Node("eskf_node") {
    ...
    p0_theta_ = declare_parameter<double>("p0_theta", 1e-3);
  ...
  void finish() {
    ...
    // 还压在队列里的帧全部放行（不再等括号观测），保证行数与 IMU 帧数一致
    ...
    if (fout_.is_open()) {
      ...
      RCLCPP_INFO(get_logger(), "b_g = (%.6f, %.6f, %.6f) rad/s，末帧 P 对角线 = (%.3e, %.3e, "
                  ...
                  last_p_[4], last_p_[5]);
      RCLCPP_INFO(get_logger(),
                  "b_a = (%.6f, %.6f, %.6f) m/s²，末帧 p = (%.4f, %.4f, %.4f) m，"
                  "v = (%.4f, %.4f, %.4f) m/s",
                  ba_[0], ba_[1], ba_[2], p_[0], p_[1], p_[2], v_[0], v_[1], v_[2]);
      RCLCPP_INFO(get_logger(),
 ...
 private:
  ...
  void on_imu(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    // 消息里是 g 单位（题面约定，imu_player_node 原样转发）
  ...
  void on_pose(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
    ...
    // 4*6+4=28、5*6+5=35 —— 不是 33/44/55（那两个超出 36 个元素的范围）。
  ...
  // 初始化（前 bias_window_frames 帧均值）：
  //   陀螺均值 → b_g；比力均值方向 → 初始 roll/pitch（yaw 不可观，设 0）。
  //   加计零偏 b_a = |g|·(f̄ − R₀ᵀ·(0,0,1))：静止时比力等于重力在该姿态下的理论投影，
  //   实测比力减去它就是零偏。实测 f̄ 的模长只有 0.9943（差 0.57%），这一项几乎全被
  //   z 轴吸收（≈0.0058 g = 0.057 m/s²）—— 它其实是加计的尺度因子误差，这里按零偏建模，
  //   是选做路径的一处近似。
  void start() {
    ...
    // 列名与单位：角度为度；四元数标量在前；位置 m、速度 m/s、b_a m/s²、b_g rad/s；
    //   残差 zp_*、NIS、S 与 P 都是极小量（P0=1e-8、S≈1e-7），用科学计数法写；
    //   upd_p 标记本帧是否做了位置更新。
    fout_ << "time,qw,qx,qy,qz,roll_deg,pitch_deg,yaw_deg,"
             "x,y,z,vx,vy,vz,ba_x,ba_y,ba_z,"
    ...
    RCLCPP_INFO(get_logger(),
                ...
                "pitch = %.3f°；静止段比力均值 = (%.6f, %.6f, %.6f) g，"
                "b_a 初值 = (%.6f, %.6f, %.6f) m/s²",
                n, bg0[0], bg0[1], bg0[2], e0.roll * 180.0 / M_PI, e0.pitch * 180.0 / M_PI,
                fa[0], fa[1], fa[2], ba0[0], ba0[1], ba0[2]);
    RCLCPP_INFO(get_logger(),
                "单位约定：加计读数 × %.5f → m/s²，重力向量取 (0, 0, +%.5f)", g_ms2_, g_ms2_);
    drain();
  }
  ...
  bool resolve_obs(double t, Obs& obs) {
    ...
    switch (pose_align_.lookup(t, po)) {
      ...
      case eskf_imu::PoseAligner::kNoObsYet:
        // t 早于最早的姿态样本（实测是开头几十帧，其中还含协方差尚未初始化的帧），
        // 永远等不到括号：无观测放行
        return true;
  ...
  void step(const Sample& s, const Obs& obs) {
    ...
    if (have_last_t_) {
      dt = s.t - last_t_;
      if (dt <= 0.0) {  // 时间戳回退/重复：跳过该帧，保证行序严格按时间递增
        ++skipped_dt_;
        last_t_ = s.t;
        return;
      }
    ...
    for (int i = 0; i < 3; ++i) {
      bg_[i] = bg[i];
      // 姿态/零偏块的方差：写成 6 维时的下标顺序（3 个 δθ + 3 个 δb_g）
    ...
    // 位置/速度 fixed 6（m 量级，1e-6 m 足够）就够了。
    fout_ << std::fixed << std::setprecision(9) << s.t;
    fout_ << "," << q.w << "," << q.x << "," << q.y << "," << q.z;
    fout_ << std::setprecision(6) << "," << e.roll * 180.0 / M_PI << "," << e.pitch * 180.0 / M_PI
          << "," << e.yaw * 180.0 / M_PI;
    fout_ << "," << p_[0] << "," << p_[1] << "," << p_[2] << "," << v_[0] << "," << v_[1] << ","
          << v_[2];
    // 零偏、位置残差、S、P 都是极小量，必须用科学计数法，
